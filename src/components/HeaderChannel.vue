<script setup>
    ...
    const channelLeft = useGenID(useHeaderChannelStore().channelLeft.map((v, i, arr) => {
            Object.assign(v, {classes: useHeaderChannelStore().channelLeftClasses[i], dir: i < (arr.length + 1) / 2 ? 'up' : 'down'})
            return v
    }))
    const { more } = useHeaderChannelStore()
    ...
    const headerChannel = ref(null)
    const { headerChannelVisible } = storeToRefs(useHeaderChannelStore())
    const observer = new IntersectionObserver(e => headerChannelVisible.value = e[0].isIntersecting)
    onMounted(() => {
        observer.observe(headerChannel.value)
    })
    
</script>

<template>
    <div ref="headerChannel" class="header-channel">
        ...
        <div class="channels-left">
            <div :ref="el => useAreaIn(el, inChannelArea[item.id - 1], 200, 100)" v-for="item in channelLeft" :key="item.id" class="column-shell-container">
                <a href="">{{ item.name }}</a>
            ...
            <div :ref="el => useAreaIn(el, inMoreArea, 200, 100)" class="more column-shell-container">
...
<style>
    ...
    .header-channel .channel-icons{
        display: flex;
        align-items: center;
        margin-right: 20px;
    }
    .header-channel .channel-icons .icon{
        display: flex;
        flex-direction: column;
        align-items: center;
        margin-right: 24px;
    }
    .header-channel .channel-icons .icon > div{
        width: 46px;
        height: 46px;
        border-radius: 50%;
        text-align: center;
    ...
    .header-channel .channel-icons .icon .dynamic-bg{
        background-color: #FF9212;
        transition: all .3s;
    }
    .header-channel .channel-icons .icon .dynamic-bg:hover{
        background-color: #FF9E2C;
    }
    .header-channel .channel-icons .icon svg{
        height: inherit;
        width: 25px;
    }
    .header-channel .channel-icons .icon .popular-bg{
        background-color: #F07775;
        transition: all .3s;
    }
    .header-channel .channel-icons .icon .popular-bg:hover{
        background-color: #F38E8C;
    }
    .header-channel .channels-left{
        flex: 1;
        display: grid;
        grid-gap: 10px;
    ...
    .header-channel .channels-left .more{
        cursor: default;
    }
    .header-channel .channels-left .more svg{
        margin-left: 5px;
        transition: all .3s;
    }
    ...
    .header-channel .channels-right > :nth-child(3n){
        letter-spacing: 0;
    }
    .header-channel .channels-right svg{
        width: 20px;
        height: 20px;
    ...
    .header-channel .channels-right a:hover{
        color: var(--link1);
    }
</style>