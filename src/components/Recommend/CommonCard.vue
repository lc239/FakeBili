<script setup>
    import { useAreaIn } from '@/js/mouse'
    import { useUnit } from '@/js/utils'
    import { reactive, ref, watch } from 'vue'
    import instance from '@/conf/axios-instance'
    import dashjs from 'dashjs'

    const props = defineProps({
        name: String,
        cover: String,
        playCount: Number,
        bulletCount: Number,
        thumbCount: Number,
        duration: String,
        uploaderName: String,
        uploadDate: String,
        path: String
    })

    const visualArea = ref(null)
    const inVisualArea = reactive({in: false})

    watch(visualArea, v => {
        useAreaIn(v, inVisualArea, 400)
    })

    let player = null
    function play(el){
        if(!el){
            player.pause()
            player.seek(0)
            return
        }
        if(!player){
            // 初始化
            player = dashjs.MediaPlayer().create()
            player.initialize()
            player.attachSource(instance.getUri() + props.path)
        }
        player.attachView(el)
        player.setMute(true)
        player.play()
    }
</script>

<template>
    <div class="common-card">
        <a :ref="el => visualArea = el" class="visual" href="">
            <Transition name="fade"><img class="cover" :src="cover" v-show="!inVisualArea.in"></Transition>
            <video v-if="inVisualArea.in" class="video" :ref="el => play(el)"></video>
            <Transition name="fade">
                <div class="bottom" v-show="!inVisualArea.in">
                    <div class="left">
                        <div class="play-count">
                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 24 24" width="24" height="24" fill="#ffffff">
                                <path d="M12 4.99805C9.48178 4.99805 7.283 5.12616 5.73089 5.25202C4.65221 5.33949 3.81611 6.16352 3.72 7.23254C3.60607 8.4998 3.5 10.171 3.5 11.998C3.5 13.8251 3.60607 15.4963 3.72 16.76355C3.81611 17.83255 4.65221 18.6566 5.73089 18.7441C7.283 18.8699 9.48178 18.998 12 18.998C14.5185 18.998 16.7174 18.8699 18.2696 18.74405C19.3481 18.65655 20.184 17.8328 20.2801 16.76405C20.394 15.4973 20.5 13.82645 20.5 11.998C20.5 10.16965 20.394 8.49877 20.2801 7.23205C20.184 6.1633 19.3481 5.33952 18.2696 5.25205C16.7174 5.12618 14.5185 4.99805 12 4.99805zM5.60965 3.75693C7.19232 3.62859 9.43258 3.49805 12 3.49805C14.5677 3.49805 16.8081 3.62861 18.3908 3.75696C20.1881 3.90272 21.6118 5.29278 21.7741 7.09773C21.8909 8.3969 22 10.11405 22 11.998C22 13.88205 21.8909 15.5992 21.7741 16.8984C21.6118 18.7033 20.1881 20.09335 18.3908 20.23915C16.8081 20.3675 14.5677 20.498 12 20.498C9.43258 20.498 7.19232 20.3675 5.60965 20.2392C3.81206 20.0934 2.38831 18.70295 2.22603 16.8979C2.10918 15.5982 2 13.8808 2 11.998C2 10.1153 2.10918 8.39787 2.22603 7.09823C2.38831 5.29312 3.81206 3.90269 5.60965 3.75693z" fill="currentColor"></path>
                                <path d="M14.7138 10.96875C15.50765 11.4271 15.50765 12.573 14.71375 13.0313L11.5362 14.8659C10.74235 15.3242 9.75 14.7513 9.75001 13.8346L9.75001 10.1655C9.75001 9.24881 10.74235 8.67587 11.5362 9.13422L14.7138 10.96875z" fill="currentColor"></path>
                            </svg>
                            <span>{{ useUnit(playCount, 1) }}</span>
                        </div>
                        <div class="bullet-count">
                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 24 24" width="24" height="24" fill="#ffffff">
                                <path d="M12 4.99805C9.48178 4.99805 7.283 5.12616 5.73089 5.25202C4.65221 5.33949 3.81611 6.16352 3.72 7.23254C3.60607 8.4998 3.5 10.171 3.5 11.998C3.5 13.8251 3.60607 15.4963 3.72 16.76355C3.81611 17.83255 4.65221 18.6566 5.73089 18.7441C7.283 18.8699 9.48178 18.998 12 18.998C14.5185 18.998 16.7174 18.8699 18.2696 18.74405C19.3481 18.65655 20.184 17.8328 20.2801 16.76405C20.394 15.4973 20.5 13.82645 20.5 11.998C20.5 10.16965 20.394 8.49877 20.2801 7.23205C20.184 6.1633 19.3481 5.33952 18.2696 5.25205C16.7174 5.12618 14.5185 4.99805 12 4.99805zM5.60965 3.75693C7.19232 3.62859 9.43258 3.49805 12 3.49805C14.5677 3.49805 16.8081 3.62861 18.3908 3.75696C20.1881 3.90272 21.6118 5.29278 21.7741 7.09773C21.8909 8.3969 22 10.11405 22 11.998C22 13.88205 21.8909 15.5992 21.7741 16.8984C21.6118 18.7033 20.1881 20.09335 18.3908 20.23915C16.8081 20.3675 14.5677 20.498 12 20.498C9.43258 20.498 7.19232 20.3675 5.60965 20.2392C3.81206 20.0934 2.38831 18.70295 2.22603 16.8979C2.10918 15.5982 2 13.8808 2 11.998C2 10.1153 2.10918 8.39787 2.22603 7.09823C2.38831 5.29312 3.81206 3.90269 5.60965 3.75693z" fill="currentColor"></path>
                                <path d="M15.875 10.75L9.875 10.75C9.46079 10.75 9.125 10.4142 9.125 10C9.125 9.58579 9.46079 9.25 9.875 9.25L15.875 9.25C16.2892 9.25 16.625 9.58579 16.625 10C16.625 10.4142 16.2892 10.75 15.875 10.75z" fill="currentColor"></path>
                                <path d="M17.375 14.75L11.375 14.75C10.9608 14.75 10.625 14.4142 10.625 14C10.625 13.5858 10.9608 13.25 11.375 13.25L17.375 13.25C17.7892 13.25 18.125 13.5858 18.125 14C18.125 14.4142 17.7892 14.75 17.375 14.75z" fill="currentColor"></path>
                                <path d="M7.875 10C7.875 10.4142 7.53921 10.75 7.125 10.75L6.625 10.75C6.21079 10.75 5.875 10.4142 5.875 10C5.875 9.58579 6.21079 9.25 6.625 9.25L7.125 9.25C7.53921 9.25 7.875 9.58579 7.875 10z" fill="currentColor"></path>
                                <path d="M9.375 14C9.375 14.4142 9.03921 14.75 8.625 14.75L8.125 14.75C7.71079 14.75 7.375 14.4142 7.375 14C7.375 13.5858 7.71079 13.25 8.125 13.25L8.625 13.25C9.03921 13.25 9.375 13.5858 9.375 14z" fill="currentColor"></path>
                            </svg>
                            <span>{{ useUnit(bulletCount, 1) }}</span>
                        </div>
                    </div>        
                    <div class="right">
                        <div class="duration">
                            <span>{{ duration }}</span>
                        </div>
                    </div>
                </div>
            </Transition>
        </a>
        <div class="description">
            <div class="title">
                <a href="">{{ name }}</a>
            </div>
            <div class="update-info">
                <span class="thumb" v-if="thumbCount >= 10000">{{ useUnit(thumbCount, 0) }}点赞</span>
                <a href="">
                    <svg v-if="thumbCount < 10000" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 24 24" width="24" height="24" fill="currentColor">
                        <path d="M6.15 8.24805C6.5642 8.24805 6.9 8.58383 6.9 8.99805L6.9 12.7741C6.9 13.5881 7.55988 14.248 8.3739 14.248C9.18791 14.248 9.8478 13.5881 9.8478 12.7741L9.8478 8.99805C9.8478 8.58383 10.1836 8.24805 10.5978 8.24805C11.012 8.24805 11.3478 8.58383 11.3478 8.99805L11.3478 12.7741C11.3478 14.41655 10.01635 15.748 8.3739 15.748C6.73146 15.748 5.4 14.41655 5.4 12.7741L5.4 8.99805C5.4 8.58383 5.73578 8.24805 6.15 8.24805z" fill="currentColor"></path>
                        <path d="M12.6522 8.99805C12.6522 8.58383 12.98795 8.24805 13.4022 8.24805L15.725 8.24805C17.31285 8.24805 18.6 9.53522 18.6 11.123C18.6 12.71085 17.31285 13.998 15.725 13.998L14.1522 13.998L14.1522 14.998C14.1522 15.4122 13.8164 15.748 13.4022 15.748C12.98795 15.748 12.6522 15.4122 12.6522 14.998L12.6522 8.99805zM14.1522 12.498L15.725 12.498C16.4844 12.498 17.1 11.8824 17.1 11.123C17.1 10.36365 16.4844 9.74804 15.725 9.74804L14.1522 9.74804L14.1522 12.498z" fill="currentColor"></path><path d="M12 4.99805C9.48178 4.99805 7.283 5.12616 5.73089 5.25202C4.65221 5.33949 3.81611 6.16352 3.72 7.23254C3.60607 8.4998 3.5 10.171 3.5 11.998C3.5 13.8251 3.60607 15.4963 3.72 16.76355C3.81611 17.83255 4.65221 18.6566 5.73089 18.7441C7.283 18.8699 9.48178 18.998 12 18.998C14.5185 18.998 16.7174 18.8699 18.2696 18.74405C19.3481 18.65655 20.184 17.8328 20.2801 16.76405C20.394 15.4973 20.5 13.82645 20.5 11.998C20.5 10.16965 20.394 8.49877 20.2801 7.23205C20.184 6.1633 19.3481 5.33952 18.2696 5.25205C16.7174 5.12618 14.5185 4.99805 12 4.99805zM5.60965 3.75693C7.19232 3.62859 9.43258 3.49805 12 3.49805C14.5677 3.49805 16.8081 3.62861 18.3908 3.75696C20.1881 3.90272 21.6118 5.29278 21.7741 7.09773C21.8909 8.3969 22 10.11405 22 11.998C22 13.88205 21.8909 15.5992 21.7741 16.8984C21.6118 18.7033 20.1881 20.09335 18.3908 20.23915C16.8081 20.3675 14.5677 20.498 12 20.498C9.43258 20.498 7.19232 20.3675 5.60965 20.2392C3.81206 20.0934 2.38831 18.70295 2.22603 16.8979C2.10918 15.5982 2 13.8808 2 11.998C2 10.1153 2.10918 8.39787 2.22603 7.09823C2.38831 5.29312 3.81206 3.90269 5.60965 3.75693z" fill="currentColor"></path>
                    </svg>
                    <span>{{ uploaderName }}</span>
                    <span class="point">·</span>
                    <span>{{ uploadDate }}</span>
                </a>
            </div>
            
        </div>
    </div>
</template>

<style>
    .common-card{
        --title-line-height: 22px;
        --update-info-line-height: 17px;
        --bottom-line-height: 18px;
    }
    .common-card .visual{
        display: block;
        position: relative;
        border-radius: 6px;
        overflow: hidden;
        height: var(--visual-cover-radio);
        width: 100%;
        background-color: black;
    }
    .common-card .visual .bottom{
        position: absolute;
        bottom: 0;
        padding: 16px 8px 5px;
        color: white;
        width: 100%;
        display: flex;
        justify-content: space-between;
        font-size: 13px;
        line-height: var(--bottom-line-height);
        background-image: linear-gradient(180deg,rgba(0,0,0,0) 0%,rgba(0,0,0,.8) 100%);
    }
    .common-card .visual .bottom .left{
        display: flex;
        align-items: center;
    }
    .common-card .visual .bottom .left > div{
        display: flex;
        align-items: center;
        margin-right: 12px;
    }
    .common-card .visual .bottom svg{
        width: var(--bottom-line-height);
        height: var(--bottom-line-height);
        margin-right: 2px;
    }
    .common-card .visual .cover, .common-card .visual .video{
        width: 100%;
        height: 100%;
        object-fit: cover;
    }
    .common-card .description{
        margin-top: 10px;
    }
    .common-card .description a{
        transition: color .2s;
    }
    .common-card .description a:hover{
        color: #00AEEC;
    }
    .common-card .description .title{
        line-height: var(--title-line-height);
        height: calc(2 * var(--title-line-height));
        padding-right: 24px;
        color: var(--text2);
        overflow: hidden;
        display: -webkit-box;
        -webkit-line-clamp: 2;
        -webkit-box-orient: vertical;
        font-family: PingFang SC,HarmonyOS_Medium,Helvetica Neue,Microsoft YaHei,sans-serif;
        font-size: 15px;
        font-weight: 500;
        transition: color .2s;
    }
    .common-card .description .update-info{
        color: var(--grey1);
        font-size: 13px;
        line-height: var(--update-info-line-height);
        margin-top: 4px;
    }
    .common-card .description .update-info .thumb{
        background-color: #FFF0E3;
        color: #FF7F24;
        font-size: 12px;
        padding: .5px 4px;
        border-radius: 4px;
        margin-right: 4px;
    }
    .common-card .description .update-info svg{
        height: var(--update-info-line-height);
        width: var(--update-info-line-height);
        margin-right: 2px;
        vertical-align: sub;
    }
    .common-card .description .update-info .point{
        margin: 0 4px;
    }
</style>